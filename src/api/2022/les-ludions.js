...
export default {
    ...
    methods: {
        ...
        async getLabels() {
            ...
            return db.collection("2022Labels")
                .get()
                .then((querySnapshot) => {
        ...
        async getEvents() {
            ...
            return db.collection("2022Evenements")
                .get()
                .then((querySnapshot) => {
        ...
        async getReservations() {
            ...
            return db.collection("2022Reservations")
                .get()
                .then((querySnapshot) => {
                    querySnapshot.forEach((res) => {
                        ...
                        db.collection("2022Reservations").doc(res.id).collection('Spectacles').get().then(
                            ss => {
                                var spectacles = [];
        ...
        getArtistes() {
            ...
            return db.collection("2022Artistes")
                .get()
                .then((querySnapshot) => {
                    querySnapshot.forEach((art) => {
                        ...
                        db.collection("2022Artistes").doc(art.id).collection('LivreDOr').get().then(
                            livreDOR => {
                                var cc = [];
                                livreDOR.forEach(c => {
                                    cc.push({
        ...
        async addComment(comment, artisteId) {
            return await db.collection("2022Artistes").doc(artisteId).collection('LivreDOr').add(comment);
        },
        ...
        async addDemande(demande, jour, label) {
          ...
          <p><strong>Spectacle </strong>: ` + demande.Places + ' places pour ' + label + ' le ' + jour + `</p>`,
            ...
            return await db.collection("2022Reservations").doc(jour).collection('Spectacles').doc(demande.Spectacle).collection('Demandes').add(demande);
        },
            ...
            if (!this.blogs || (this.blogs && !this.blogs.isLoading)) {
                ...
                blogs.forEach(
                    async blog => {
                        ...
                        let blocs = await db.collection("2022Blog").doc(blog.id).collection("Paragraphes").get();
                        blocs.forEach(
                            async bloc => {
        ...
        async getArticle(id) {
            ...
            if (!this.article || !this.article.isLoading || (this.article.value.id != id)) {
                ...
                let art = await db.collection("2022Blog").doc(id).get();
                ...
                let blocs = await db.collection("2022Blog").doc(art.id).collection("Paragraphes").get();
                blocs.forEach(
                    async bloc => {
                        let dataBloc = bloc.data();
                        var tempBloc = {
