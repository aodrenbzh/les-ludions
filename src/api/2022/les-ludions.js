...
export default {
  ...
  methods: {
    ...
    async getLabels() {
      var that = this;
      if (this.labels && this.labels.isLoading)
        return Promise.resolve(this.labels.value);
      this.labels = {
        value: {},
        isLoading: true
      };
      return db
        .collection("2022Labels")
        .get()
        .then(querySnapshot => {
    ...
    async getEvents() {
      var that = this;
      if (this.events && this.events.isLoading)
        return Promise.resolve(this.events.value);
      this.events = {
        value: [],
        isLoading: true
      };
      return db
        .collection("2022Evenements")
        .get()
        .then(querySnapshot => {
          querySnapshot.forEach(event => {
            ...
            var storageRef = str.ref();
            var imagesRef = storageRef.child("/events2022");
    ...
    async getReservations() {
      var that = this;
      if (this.reservations && this.reservations.isLoading)
        return Promise.resolve(this.reservations.value);
      this.reservations = {
        value: {},
        isLoading: true
      };
      return db
        .collection("2022Reservations")
        .get()
        .then(querySnapshot => {
          querySnapshot.forEach(res => {
            ...
            db.collection("2022Reservations")
              .doc(res.id)
    ...
    getArtistes() {
      var that = this;
      if (this.artistes && this.artistes.isLoading)
        return Promise.resolve(this.artistes.value);
      this.artistes = {
        value: [],
        isLoading: true
      };
      return db
        .collection("2022Artistes")
        .get()
        .then(querySnapshot => {
          querySnapshot.forEach(art => {
            ...
            var storageRef = str.ref();
            var imagesRef = storageRef.child("/2022" + art.id);
            imagesRef.listAll().then(images => {
              var tempI = [];
              images.items.forEach(itemRef => {
                itemRef.getMetadata().then(meta => {
    ...
    async getImagesFromArtiste(id) {
      ...
      var result = [];
      var storageRef = str.ref();
      var imagesRef = storageRef.child("/2022" + id);
      let images = await imagesRef.listAll();
      images.items.forEach(async itemRef => {
        let meta = await itemRef.getMetadata();
        let url = await storageRef.child(meta.fullPath).getDownloadURL();
      ...
      this.artistes.value[
        this.artistes.value.findIndex(a => a.id == id)
      ].galerie = result;
    ...
    async sendPigeon(pigeon) {
      ...
      return await db.collection("2022Pigeons").add(input);
    },
    async addDemande(demande, jour, label) {
      var input = {
        ...
        message: {
          subject:
            "Reservation reçue de la part de " +
            demande.Prenom +
            " " +
            demande.Nom,
          text:
      ...
      await db.collection("2022Pigeons").add(input);
      return await db
        .collection("2022Reservations")
        .doc(jour)
        .collection("Spectacles")
        .doc(demande.Spectacle)
        .collection("Demandes")
        .add(demande);
    },
    ...
    async getBlogs() {
      ...
      if (!this.blogs || (this.blogs && !this.blogs.isLoading)) {
        ...
        let allBlogs = [];
        let blogs = await db.collection("2022Blog").get();
        blogs.forEach(async blog => {
          ...
          let imageBlogRef = await str
            .ref()
            .child("blogs/" + blog.id)
            .listAll();
          imageBlogRef.items.forEach(async i => {
    ...
    async getArticle(id) {
      ...
      if (
        !this.article ||
        !this.article.isLoading ||
        this.article.value.id != id
      ) {
        this.article = {
          value: {},
          isLoading: true
        ...
        tempArt.blocs = tempArt.blocs.sort((a, b) =>
